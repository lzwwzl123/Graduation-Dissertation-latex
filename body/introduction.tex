...
\bicaption{}{}{打高尔夫球球的人（博士论文双语题注）}{Fig.$\!$}{The person playing golf (Doctoral thesis)}
\label{golfer1}
\end{figure}
...
\subsection{引用图示例}[Cited figure example]
如\cref{fig:cited-golfer}所示，引用文献中的图片时，可以在正文中先说明图片来源，
并在图题中补上参考文献标注，这样图号和文献编号都可以点击跳转。
\begin{figure}[htbp]
\centering
\includegraphics[width = 0.4\textwidth]{golfer}
\caption{引自文献的示意图\protect\cite{cnarticle}}
\label{fig:cited-golfer}
\end{figure}
...
\bicaption{}{}{打高尔夫球的人}{Fig.$\!$}{The person playing golf}
\label{golfer2}
\end{minipage}
\centering
\begin{minipage}{0.4\textwidth}
\centering
\includegraphics[width=\textwidth]{golfer}
\bicaption{}{}{打高尔夫球的人。注意，这里默认居中}{Fig.$\!$}{The person playing golf. Please note that, it is vertically center aligned by default.}
\label{golfer3}
\end{minipage}
\end{figure}
...
\centering
\includegraphics[width=\textwidth]{golfer}
\bicaption{}{}{打高尔夫球的人。注意，此图是顶部对齐}{Fig.$\!$}{The person playing golf. Please note that, it is vertically top aligned.}
\label{golfer8}
\end{minipage}
\end{figure}
...
\bicaption{}{}{打高尔夫球的人。注意，此图对齐方式是图片底部对齐}{Fig.$\!$}{The person playing golf. Please note that, it is vertically bottom aligned for figure.}
\label{golfer9}
\end{minipage}
\centering
\begin{minipage}[t]{0.4\textwidth}
...
\end{minipage}
\vspace{0.2em}
\bicaption{}{}{打高尔夫球的人}{Fig.$\!$}{The person playing gol}
\label{golfer4}
\end{figure}
...
\centering
\begin{tikzpicture}
  \node[inner sep=0] (image) {\includegraphics[width=0.3\textwidth]{golfer}};
  \node[fill=white,inner sep=1pt] at ([xshift=-2.0cm,yshift=1.4cm]image.south east) {(a)};
  \node[fill=white,inner sep=1pt] at ([xshift=-0.7cm,yshift=0.6cm]image.south east) {(b)};
\end{tikzpicture}
\bicaption{}{}{打高尔夫球的人（TikZ 标注示例）}{Fig.$\!$}{The person playing golf (TikZ annotation example)}
\label{golfer0}
\vskip -0.4em
 \hspace{2em}
...
\begin{figure}[!htbp]
  ...
  \begin{sideways}
    \begin{minipage}{\textheight}
      ...
      \fbox{\includegraphics[width=0.2\textwidth]{golfer}}
      \fbox{\includegraphics[width=0.2\textwidth]{golfer}}
\bicaption{}{}{打高尔夫球的人（非规范要求）}{Fig.$\!$}{The person playing golf (Not stated in the regulation)}
\label{golfer7}
    \end{minipage}
  \end{sideways}
\end{figure}
...
\begin{figure}[thb!]
  \centering
  \begin{tikzpicture}[x=1cm,y=1cm,every node/.style={font=\wuhao}]
    \node[draw,rounded corners,fill=gray!10,minimum width=2.4cm,minimum height=0.8cm] (ref) at (0,2.4) {参考基因组};
    \node[draw,rounded corners,fill=blue!8,minimum width=2.4cm,minimum height=0.8cm] (sample) at (0,1.0) {肿瘤样本};
    \node[draw,rounded corners,fill=green!8,minimum width=2.4cm,minimum height=0.8cm] (normal) at (-2.3,-0.5) {正常细胞};
    \node[draw,rounded corners,fill=red!8,minimum width=2.4cm,minimum height=0.8cm] (tumor) at (2.3,-0.5) {肿瘤细胞};
    \node[draw,circle,fill=red!12,minimum size=0.8cm] (c1) at (0,-1.8) {1};
    \node[draw,circle,fill=red!12,minimum size=0.8cm] (c2) at (2.3,-1.8) {2};
    \node[draw,circle,fill=red!12,minimum size=0.8cm] (c3) at (-2.3,-3.1) {3};
    \node[draw,circle,fill=red!12,minimum size=0.8cm] (c4) at (0,-3.1) {4};
    \node[draw,circle,fill=red!12,minimum size=0.8cm] (c5) at (2.3,-3.1) {5};
    \draw[->,thick] (ref) -- (sample);
    \draw[->,thick] (sample) -- (normal);
    \draw[->,thick] (sample) -- (tumor);
    \draw[->,thick] (tumor) -- (c1);
    \draw[->,thick] (tumor) -- (c2);
    \draw[->,thick] (c1) -- (c3);
    \draw[->,thick] (c1) -- (c4);
  ...
  \begin{minipage}{.9\linewidth}
      \vskip 0.2em
      \wuhao 图中，箭头表示肿瘤子种群的演化关系。该示例保留了 TikZ 作图的层次结构展示，同时避免了复杂样式定义带来的额外编译告警。
      \vspace{0.6em}
  \end{minipage}
\bicaption{}{}{肿瘤组织中各个子种群的进化示意图}{Fig.$\!$}{The diagram of tumor subpopulation evolution process}
\label{tumor}
\end{figure}
...
表格应具有三线表格式，因此需要调用~booktabs~宏包，其标准格式如\cref{table1}所示。
\begin{table}[htbp]
\bicaption{}{}{符合研究生院绘图规范的表格}{Table$\!$}{Table in agreement of the standard from graduate school}
\label{table1}
...
\verb|longtable|环境，所绘制的长表格的格式如\cref{table2}所示。
...
\cref{table2}第~2~页的标题“编号（续表）”和表头是通过代码自动添加上去的，无需人工添加，若表格在页面中的竖直位置发生了变化，长表格在第~2~页
...
首先给出这种情况下的一个例子如\cref{table3}所示。
\begin{table}[htbp]
  \centering
\bicaption{}{}{最小的三个正整数的英文表示法}{Table$\!$}{The English construction of the smallest three positive integral numbers}\vspace{0.5em}\wuhao
\label{table3}
...
\verb|tabularx|环境生成的对公式中物理量符号进行注释的公式如\cref{eq:1}所示。
...
\begin{sideways}
\begin{minipage}{\textheight}
\bicaption{}{}{不在规范中规定的横版表格}{Table$\!$}{A table style which is not stated in the regulation}
\label{table4}
\vspace{0.5em}\centering\wuhao
\begin{tabular}{ccccc}
\toprule
...
。
\subsection{中文伪斜体}[Fake slant for Chinese]
中文默认写法：这是普通宋体中文效果，用来和伪斜体做对比。
中文伪斜写法：\hitsongfakeit{这是使用 FakeSlant 的宋体伪斜体效果，适合在没有真斜体字库时做替代强调。}
...
\section{关于捐助}
各位刀客和大侠如用的嗨，要解囊相助，请参照\cref{zfb}中提示操作（二维码被矢量化后之后去
除了头像等冗余无用的部分～）。

\begin{figure}[!htbp]
\centering\includegraphics[width=0.4\textwidth]{zfb}
\vspace{0.2em}
\bicaption{}{}{捐助，注意此处是子图只用汉语图题的形式，我工规定可以不用
